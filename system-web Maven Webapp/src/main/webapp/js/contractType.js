...
function page_init(params) {
	$("#contractType").datagrid({
				...
				onDblClickRow : function(index, row) {
					$("#contractTypeInfo").form('load', row);
...
function openDialog(e, row) {
	$("#contractTypeInfo").form('reset');
	title = "新增";
	if ($(e).attr("tag") == "edit") {
		var row = $('#contractType').datagrid('getSelected');
		if (row) {
			$("#contractTypeInfo").form('load', row);
			$("#systemId").val(row.systemInfo.id);
			title = "编辑";
...
function doEdit() {
	var data = $("#contractTypeInfo").serializeJson();
	$.ajax({
				url : '../print/updateContractType',
				type : 'post',
