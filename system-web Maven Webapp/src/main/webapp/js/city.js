...
function openDialog(e, row) {
	...
	title = "新增";
	if ($(e).attr("tag") == "edit") {
		var row = $('#city').datagrid('getSelected');
		if (row) {
			$("#cityInfo").form('load', row);
			$("#systemId").val(row.systemInfo.id);
			title = "编辑";
...
function doEdit() {
	var data = $("#cityInfo").serializeJson();
	console.log(data)
	$.ajax({
				url : '../print/updateCity',
				type : 'post',
